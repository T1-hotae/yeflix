...
import { LogOut, ChevronLeft, User, Globe, Moon, Info } from 'lucide-react';
    ...
    return (
      <div className="min-h-screen bg-cinema-bg flex items-center justify-center">
        <div className="text-center">
          ...
          <button onClick={loginWithGoogle} className="bg-white text-gray-900 font-semibold px-6 py-2.5 rounded-full text-sm">
            Google로 로그인
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-cinema-bg">
      <div className="max-w-2xl mx-auto px-4 py-10">
        <div className="flex items-center gap-3 mb-8">
          <button onClick={() => navigate(-1)} className="text-cinema-muted hover:text-white transition">
            <ChevronLeft size={22} />
          </button>
        ...
        <div className="bg-cinema-card rounded-2xl p-6 border border-white/5 mb-4">
          <p className="text-xs text-cinema-muted uppercase tracking-wider font-semibold mb-4">내 계정</p>
          <div className="flex items-center gap-4">
            <img src={user.photoURL} alt="프로필" className="w-16 h-16 rounded-full border-2 border-cinema-gold/30" />
            <div>
        ...
        <div className="bg-cinema-card rounded-2xl border border-white/5 mb-4 overflow-hidden">
          <p className="text-xs text-cinema-muted uppercase tracking-wider font-semibold px-6 pt-5 pb-3">앱 설정</p>
          <SettingRow icon={<Globe size={16} />} label="언어" value="한국어" />
          <SettingRow icon={<Moon size={16} />} label="테마" value="다크 모드" />
        </div>

        {/* 정보 */}
        <div className="bg-cinema-card rounded-2xl border border-white/5 mb-6 overflow-hidden">
          <p className="text-xs text-cinema-muted uppercase tracking-wider font-semibold px-6 pt-5 pb-3">정보</p>
          <SettingRow icon={<Info size={16} />} label="버전" value="1.0.0" />
          <SettingRow icon={<Info size={16} />} label="영화 데이터" value="TMDB 제공" />
          <SettingRow icon={<Info size={16} />} label="스트리밍 정보" value="JustWatch 제공" />
        </div>
